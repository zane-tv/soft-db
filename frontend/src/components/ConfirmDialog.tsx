...
export function ConfirmDialog({
  ...
  useEffect(() => {
    if (open) {
      // Focus cancel by default for safety, but keep confirm accessible
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      >
        ...
        <div className="px-6 py-5 flex items-center justify-center gap-3">
          <button
            onClick={onCancel}
            className="flex-1 px-4 py-2.5 rounded-lg text-sm font-medium text-text-main bg-white/5 hover:bg-white/10 border border-border-subtle transition-all duration-200 active:scale-[0.97]"
          >
            {cancelLabel}
          </button>
          <button
