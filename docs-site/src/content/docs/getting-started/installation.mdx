...
<Tabs>
  ...
  <TabItem label="macOS">
    <Steps>
      ...
      3. Launch SoftDB from Applications or Spotlight.
    </Steps>

    ### Allowing SoftDB on macOS

    Because SoftDB is distributed outside the Mac App Store and is not yet notarized by Apple, macOS Gatekeeper will block it on first launch. You'll see a message like **"SoftDB can't be opened because Apple cannot check it for malicious software"** or **"SoftDB is damaged and can't be opened"**.
    ...
    **Method 1 — System Settings (recommended)**
    ...
    <Steps>
      1. Try to open SoftDB — macOS will block it. Click **OK** to dismiss the dialog.

      2. Open **System Settings → Privacy & Security**.

      3. Scroll down. You'll see a message: **"SoftDB was blocked from use because it is not from an identified developer."**

      4. Click **Open Anyway** and enter your password when prompted.

      5. macOS will ask one more time — click **Open** to confirm. SoftDB will launch normally from now on.
    </Steps>
    ...
    **Method 2 — Terminal (if Method 1 doesn't work)**
    ...
    ```bash
    xattr -cr /Applications/SoftDB.app
    ```

    Then open SoftDB again — it should launch without issues.

    <Aside type="tip">
      If you install via **Homebrew** (below), Gatekeeper is bypassed automatically — no extra steps needed.
    </Aside>

    ### Install via Homebrew

    The easiest way to install on macOS — no Gatekeeper prompts:
