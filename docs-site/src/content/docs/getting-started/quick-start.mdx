...
Click any database to expand it, then click a table name to open it in the data grid. Once you're inside a database, a compact **database switcher** dropdown appears at the top of the sidebar — use it to jump between databases without collapsing back to the tree view.

The grid is virtualized, so it handles large tables without freezing.
