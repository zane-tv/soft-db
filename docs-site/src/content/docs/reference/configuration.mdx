...
| Setting | Key | Default | Description |
|---------|-----|---------|-------------|
| AI provider | `aiProvider` | `"openai"` | Which AI backend to use for the AI Assistant. Options: `"openai"`, `"anthropic"`, `"ollama"`. |
| AI API key | `aiApiKey` | `""` | API key for the selected provider. Required for OpenAI and Anthropic. Leave empty when using OAuth login or Ollama. |
| AI base URL | `aiBaseUrl` | `""` | Custom API base URL. Useful for self-hosted Ollama instances or OpenAI-compatible proxies. |
